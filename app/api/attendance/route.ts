...
// Helper to get the next session number for a student based on sequence gaps
async function getNextSessionNumber(mentorName: string, studentName: string) {
  ...
  for (let i = 1; i <= Math.max(...sessionNumbers); i++) {
    if (!sessionNumbers.includes(i)) {
      console.log(`Found gap in sequence: ${sessionNumbers.join(', ')} -> next session = ${i}`);
      return i;
    }
  }
  
  // If no gaps found, return the next number after the highest
  const nextNumber = Math.max(...sessionNumbers) + 1;
  console.log(`No gaps found in sequence: ${sessionNumbers.join(', ')} -> next session = ${nextNumber}`);
  return nextNumber;
}
